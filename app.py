            ...
            try:
                ...
                if any(kw in text_lower for kw in ["น้ำหนัก", "กก", "kg"]) or text.replace(".", "", 1).isdigit():
                    ...
                    if weight_match:
                        ...
                        entry = user_drug_selection.get(user_id, {})
                        drug = entry.get("drug")

                        # ✅ ตรวจสอบว่าเป็นยาทั่วไปก่อน (อยู่ใน DRUG_DATABASE)
                        if drug in DRUG_DATABASE:
                            if "indication" not in entry:
                                reply = "❗️ กรุณาเลือกข้อบ่งใช้ก่อน เช่น 'Indication: Pneumonia'"
                            else:
                        ...
                        # ✅ ยาที่อยู่ใน SPECIAL_DRUGS และไม่ซ้ำใน DRUG_DATABASE
                        elif drug in SPECIAL_DRUGS and drug not in DRUG_DATABASE:
